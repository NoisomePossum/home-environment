...
function __bobthefish_git_branch -S -d 'Get the current git branch (or commitish)'
    ...
    and begin
        ...
        and set truncname (string replace -r '^(.{17}).{3,}(.{5})$' "\$1É\$2" $branch)
...
function __bobthefish_prompt_git -S -a git_root_dir -a real_pwd -d 'Display the actual git state'
    ...
    # only show work dir if it's a parentÉ
    if [ "$work_dir" ]
        switch $real_pwd/
            case $work_dir/\*
...
function fish_prompt -d 'bobthefish, a fish theme optimized for awesome'
    ...
    echo
    __bobthefish_start_segment 3B4252 A3BE8C
    __custom_prompt
    __bobthefish_finish_segments
end
